...
<div id="app">
    ...
    <div class="container" id="main">
        <div class="notice">
            <a  href="javascript:;" id="user_login_now">管理员</a>
        </div>
        <!--留言列表-->
        <div id="comments"><!--提示-->
            <div class="tips">
...
<script type="text/javascript">
    ...
    var editurl = "{{url('/admin/edit')}}";
    var deleteurl = "{{url('/admin/delete')}}";
    var subreviewurl = "{{url('/subreview')}}";
    var baseurl = "{{url()}}";
    var html = '';
    if($.cookie('user_name')){
        $('#user_login_now').html('管理员：'+$.cookie('user_name'));
    }
    function outlogin(){
            ...
            success: function (result) {
                ...
                if (result.data ) {
                    for(var i=0;i<result['data'].length;i++){
                        ...
                        html += '<p class="comments-text" id="comments-text-'+result['data'][i]['id']+'">'+result['data'][i]['content']+'</p>';
                        html += '<div class="info text-right">'
                        html += ' <a class="reply cursor-pointer hover-default reply_tips_off" onclick="edit('+result['data'][i]['id']+')"><i class="fa fa-comments-o"></i>&nbsp;<span>编辑</span></a>';
                        html += ' <a class="reply cursor-pointer hover-default reply_tips_off" onclick="deleteContent('+result['data'][i]['id']+')"><i class="fa fa-comments-o"></i>&nbsp;<span>删除</span></a>';
            ...
            success: function (result) {
                ...
                if (result.data) {
                    for(var i=0;i<result['data'].length;i++){
                        ...
                        html += '<div class="info text-right">';
                        html += ' <a class="reply cursor-pointer hover-default reply_tips_off" onclick="edit('+result['data'][i]['id']+')"><i class="fa fa-comments-o"></i>&nbsp;<span>编辑</span></a>';
                        html += ' <a class="reply cursor-pointer hover-default reply_tips_off" onclick="deleteContent('+result['data'][i]['id']+',1)"><i class="fa fa-comments-o"></i>&nbsp;<span>删除</span></a>';
                        html += '</div></div>';
                    }
                    $(obj).html(html);
                    $('#comment_'+id).html(1);
    ...
    function edit(id){
        ...
        text = $('#comments-text-'+id).html();
        text = prompt("请输入你的回复内容:",text);
        if(!$.trim(text)){
            alert( '请输入留言内容!');
            return false;
        }
    ...
    function deleteContent(id,child=0){
        if(!id){
            alert( '非法参数');
            return false;
        }
